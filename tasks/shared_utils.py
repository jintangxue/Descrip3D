...
def setup_model(
    ...
    # Set device properly for multi-GPU training
    local_rank = int(os.environ["LOCAL_RANK"])  # Get assigned GPU ID
    ...
    if config.distributed:
        model = torch.nn.parallel.DistributedDataParallel(
            ...
            device_ids=[local_rank],  # Each process should use only one GPU
    ...
    if config.get("auto_resume", False):
        ...
        print("osp.isfile(model_latest)", osp.isfile(model_latest))
        print(model_latest)
