...
from models.descrip3d import Descrip3D
...
from dataset.base_dataset import recover_caption
import re
...
def train(
    ...
    loss_names = ["loss", "obj_norm", "obj_img_norm", "objid_norm", "scene_norm", "obj_text_norm"]
    ...
    for i, batch in tqdm(enumerate(val_loader)):
        ...
        if "custom_prompt" in batch:
            ...
            for bi in range(batch_size):
                ...
                NOISY_DESC_RE = re.compile(r"\[Generated description \(may be noisy\)\].*", re.DOTALL)
                prompt_clean = NOISY_DESC_RE.sub("", prompt).strip()
...
def main(config):
    ...
    model_cls = eval(config.model.get('model_cls', 'Descrip3D'))
    (
        model,
        model_without_ddp,
        optimizer,
