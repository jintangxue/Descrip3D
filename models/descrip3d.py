...
class Descrip3D(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.text_feat_dim = config.model.get("text_feat_dim", 768)
        self.train_text_proj = config.model.get("train_text_proj", True)
        ...
        if not self.debug:
                ...
                def find_linear_layers(model, lora_target_modules):
                    ...
                    for name, module in model.named_modules():
                        if (
                            ...
                            and all([x not in name for x in ["instance2embed", "hidden_state2query"]])
            ...
            objid_tokens = [f"<OBJ{i:03}>" for i in range(self.max_obj_num)]
        ...
        self.object_text_proj = nn.Sequential(
            nn.Linear(self.text_feat_dim, self.llama_dim),
            nn.GELU(),
            nn.Linear(self.llama_dim, self.llama_dim)
        )
        ...
        if not self.train_text_proj:
            for p in self.object_text_proj.parameters():
                p.requires_grad = False
    ...
    def get_objid_embeds(self):
        if self.config.model.use_lora:
            objid_embeds = self.llama_model.model.model.embed_tokens.weight[self.objid_start_idx:self.objid_end_idx]
        else:
    ...
    def prepare_fixed_embed(self):
        """
        Build cached fixed system pieces using BOTH markers in instruction.txt:
          ... [<ADDON>]. Object information: [<REPLACE>].
        We cache three parts:
          p_0: text before <ADDON>
          p_mid: text between <ADDON> and <REPLACE> (e.g., "]. Object information: [")
          p_1: text after <REPLACE> (e.g., "]. USER: ")
        """
    ...
    def get_object_list_embed(self, embed_obj, embed_img, embed_scene, embed_text, scene_mask, obj_id, assigned_ids):
        ...
        if self.fuse_with_id:
            ...
            if embed_text is not None:
                object_list_embed += embed_text[assigned_ids]
            return object_list_embed
        ...
        if self.feat_fusion:
            ...
            if self.add_img_token:
                object_list_embed[1::2, :] += embed_img[assigned_ids]
            if embed_text is not None:
                object_list_embed[1::2, :] += embed_text[assigned_ids]
            return object_list_embed
        ...
        if self.no_obj:
            ...
            object_list_embed[1::3, :] = embed_img[assigned_ids]
            object_list_embed[2::3, :] = embed_text[assigned_ids]
            return object_list_embed
        ...
        if embed_img is None and embed_scene is not None:
            ...
            object_list_embed[3::4, :] = embed_text[assigned_ids]
            return object_list_embed
        ...
        if embed_img is not None and embed_scene is not None:
            object_list_embed = torch.zeros((selected_objid_embeds.shape[0] * 5, selected_objid_embeds.shape[1]),
                                            dtype=selected_objid_embeds.dtype, device=selected_objid_embeds.device)
            object_list_embed[0::5, :] = selected_objid_embeds
            object_list_embed[1::5, :] = embed_obj[assigned_ids]
            object_list_embed[2::5, :] = embed_scene[assigned_ids]
            object_list_embed[3::5, :] = embed_img[assigned_ids]
            object_list_embed[4::5, :] = embed_text[assigned_ids]
            return object_list_embed
    ...
    def forward_train(self, scene_feat, scene_img_feat, scene_locs, scene_mask, obj_ids, assigned_ids, questions, answers, is_eval=False, **kwargs):
        ...
        proj_object_img_embed = self.object_img_proj(object_embed if object_img_embed is None else object_img_embed)
        scene_text_feat = kwargs["scene_text_feat"]
        proj_object_text_embed = self.object_text_proj(scene_text_feat)
        ...
        return dict(
            ...
            obj_text_norm=proj_object_text_embed.norm(dim=-1).mean().detach().cpu(),
    ...
    def evaluate(self, scene_feat, scene_img_feat, scene_locs, scene_mask, custom_prompt, obj_ids, assigned_ids, is_eval=True, **kwargs):
        ...
        for i in range(batch_size):
            ...
            if self.bidirection:
                ...
                addon_len = 0 if addon_embed is None else addon_embed.shape[1]
                st = p_0_embed.shape[1] + addon_len + p_mid_embed.shape[1]
                ed = st + object_list_embed.shape[1]
